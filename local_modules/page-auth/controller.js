var debug = require('debug')('page-auth:controller');
...
exports = module.exports = function authController(o) {

  /**
   * Expose User gravatar
   *
   * @type {User.gravatar}
   */
  exports.gravatar = User.gravatar;
  ...
  exports.getOauthUnlink = function(req, res, next) {
    ...
    User.findById(req.user.id, function(err, user) {
      ...
      if (typeof user[provider] === 'function') {
        user[provider](undefined);
      }

      function remainigTokens() {
        return user.tokens.filter(function(token) {
          return token.kind !== provider;
        });
      }

      user.tokens(remainigTokens());
